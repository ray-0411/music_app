...
- [x] 第三階段：新增歌手前可預覽頻道頭像、名稱、channel ID 與影片總數。
- [x] 第三階段：歌手與歌曲管理改成列表加編輯按鈕，不再預設整列都是輸入框。
- [x] 第三階段：加強 CustomTkinter 元件字體套用，預設使用微軟正黑體。
- [x] 第三階段：影片頁顯示頻道影片總數，無法取得時顯示未知。
- [x] 第三階段：目前頁面影片會背景補抓上傳日期與觀看人數。
- [x] 第三階段：觀看人數顯示為千分位格式。
- [x] 圖片：新增歌手時保存頻道頭像 URL。
- [x] 圖片：歌手頭像快取到 `cache/channel_avatars/`。
- [x] 圖片：下載歌曲時同步快取歌曲縮圖。
- [x] 圖片：歌手管理列表顯示歌手頭像。
- [x] 圖片：歌曲管理列表顯示歌曲縮圖。
- [x] 圖片：歌手頭像改保存到 `assets/artists/`。
- [x] 圖片：已下載歌曲縮圖改保存到 `assets/songs/`。
- [x] 第四階段：新增歌手標籤 SQL schema。
- [x] 第四階段：預設建立上層分類：性別、團體、類型、其他。
- [x] 第四階段：標籤管理頁可新增上層與下層標籤。
- [x] 第四階段：標籤停用採 `is_available=0` 假刪除。
- [x] 第四階段：歌手編輯頁可多選每個分類的下層標籤。
- [ ] 尚未實際人工測試 YouTube 下載。
...
- 第三階段主要做 debug 與資料顯示修正。
- 頻道頭像快取到 `cache/channel_avatars/`。
- 圖片快取不進 Git，不影響資料庫與 MP3，可安全刪除後重抓。
- `assets/` 不進 Git，但屬於本機持久資料，不建議像 cache 一樣隨手刪除。
- 歌手標籤採正規化 SQL 表格，不使用 JSON 欄位。
- 每個上層分類可多選下層標籤。
