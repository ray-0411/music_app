...
class App(ctk.CTk):
    ...
    def _build_settings_page(self) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(frame, text="設定", font=self.title_font, anchor="w").grid(
            row=0, column=0, columnspan=2, sticky="ew", padx=18, pady=(18, 14)
        )
        ctk.CTkLabel(frame, text="歌曲總空間", font=self.font, anchor="w").grid(
            row=1, column=0, sticky="w", padx=18, pady=8
        )
        self.song_storage_label = ctk.CTkLabel(frame, text="-", font=self.font, anchor="w")
        self.song_storage_label.grid(row=1, column=1, sticky="ew", padx=18, pady=8)
        ctk.CTkLabel(frame, text="圖片總空間", font=self.font, anchor="w").grid(
    ...
    def refresh_settings_page(self) -> None:
        if not hasattr(self, "song_storage_label"):
            return
